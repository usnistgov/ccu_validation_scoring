Corpus Title:    CCU Phase 1 Mini-Eval Simulated L1 TA1 Annotation Reference
LDC Catalog-ID:  LDC2022R17
Date:            September 19, 2022


1. Introduction

This package constitutes a simulated TA1 annotation reference release for
the CCU Phase 1 Mini-Eval for L1. The data in this package is
representative in terms of format and content of real evaluation reference
annotation but is not actual annotation output. The purpose of this release
is to support development of TA1 evaluation scoring software and is
released to NIST only.
...
package:
  ...
  docs/                 -- contains a segments.tab file with
                           information about start/end of each annotation
                           segment; norm definition files and a
                           norm_info.tab file describing the exposed/hidden
                           status of each norm in this release; and a
                           file_info.tab document containing basic
                           information about the source data files
                           for which annotations appear in this package

  index_files/		-- contains an index.tab file listing source data 
  			   filestems for which annotations appear in this 
			   package 
...
as shown below:
  ...
  4. emotion            - emotion category present in segment (fear, anger, sadness,
                          ...
                          are present in the segment. 'noann' indicates
                          that the segment was manually or automatically
                          judged to contain no annotateable content. 
  5. multi_speaker      - annotation to indicate if multiple speakers are
                          expressing emotion in a segment: TRUE, FALSE, nosann,
...
initial line of the file provides the column labels as shown below:
  ...
  4. norm               - norm category present in segment, norms are assigned a 3-digit
                          norm_id (001, 002, 003, etc., none, or noann)
  ...
  5. status             - status of annotated norm, indicating if the norm is
                          adhered to or violated (adhere, violate, noann, or EMPTY_NA).
...
line of the file provides the column labels as shown below:
  ...
  3. segment_id         - unique identifier for the segment consisting of
                          the file_id appended by a 4-digit within-file segment
                          ID number (e.g. M010009BD_0001)
  4. valence_continuous - valence value annotated by continuous slider (1-1000 or noann)
  5. valence_binned     - scalar "bin" of valence value
                          ...
                          column #4
  6. arousal_continuous - arousal value annotated by continuous slider (1-1000 or noann)
...
README updated by Stephanie Strassel September 16, 2022

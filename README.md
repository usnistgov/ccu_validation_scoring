...
This README file describes the reference annotation validation tool, system output validation tool, scoring tool, reference statistics computing tool and fake submission generation tool.
 ...
 - Fake Submission Generation Tool: generates a random submission for the ND or ED task.
...
The CCU validation and scoring toolkit expects input directories and/or files to have specific structures and formats. This section gives more informationon these structures and formats. 

The `reference_directory` referred in subsequent sections must follow the LDC annotation data package directory structure and at a minimum must contain the following files in the given directory structure to pass validation:
...
Please refer to the `README` in the LDC CCU annotation data package for the formats of the above `.tab`
files. 
...
The toolkit uses different index files for different functions:
...
* `system output index file` - tells the scorer which files were processed by the system. This file is generated by the users and is used in validation and scoring. The format is described in the CCU Evaluation Plan.
...
An example of a `system output index file` can be found in:
```bash
...
**Validate a reference annotation directory** to make sure the `reference_directory` have the required files.
...
An example of a `system output index file` can be found in
```bash
test/pass_submissions/pass_submissions_LDC_reference_sample/ND/CCU_P1_TA1_ND_NIST_mini-eval1_20220815_164235/system_output.index.tab
```
...
Use the command below to **score an NDMAP submission directory** and an ND submission against a `reference directory` with a `scoring index file`.

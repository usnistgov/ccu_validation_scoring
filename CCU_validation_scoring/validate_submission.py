...
def global_subm_checks(task, reference_dir, submission_dir):
	...
	index_file_path, index_df, subm_file_dict = check_index_get_submission_files(ref, submission_dir)
	subm_file_length_dict = add_length_subm_file_dict(reference_dir, subm_file_dict)
	check_submission_files(submission_dir, index_file_path, index_df, subm_file_length_dict)

	return subm_file_length_dict

def individual_subm_check(task, subm_file_path, column_map, header_map, processed_label, subm_file, length):
...
def validate_nd_submission_dir_cli(args):
	...
	task = "norms"
	subm_file_length_dict = global_subm_checks(task, args.reference_dir, args.submission_dir)

	invalid_subm_file_path = []

	for subm_file in subm_file_length_dict:
		column_map, header_map = extract_modality_info(subm_file_length_dict[subm_file]["type"])
		subm_file_path = subm_file_length_dict[subm_file]["path"]
		processed_label = subm_file_length_dict[subm_file]["processed"]
		length = subm_file_length_dict[subm_file]["length"]
		
		if individual_subm_check(task, subm_file_path, column_map, header_map, processed_label, subm_file, length):
			pass
		else:
